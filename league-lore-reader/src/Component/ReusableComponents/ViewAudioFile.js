...
function ViewAudioFile({story, champions, file}) {
        ...
        <div>
            <div className="bg-white border-slate-100 dark:bg-slate-800 dark:border-slate-500 border-b rounded-t-xl p-4 pb-6 sm:p-10 sm:pb-8 lg:p-6 xl:p-10 xl:pb-8 space-y-6 sm:space-y-8 lg:space-y-6 xl:space-y-8">
                <div className="flex items-center space-x-4">
                    <img src={story.image_url} alt="" width="88" height="88" className="flex-none rounded-lg bg-slate-100"/>
                    <div className="min-w-0 flex-auto space-y-1 font-semibold">
                        <h2 className="text-slate-500 dark:text-slate-400 text-sm leading-6 truncate">
                            {champions.map((champion) => champion.name + " ")}
                        </h2>
                ...
                <div className="space-y-2">
                    <div className="relative">
                        <div className="bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                            <div className="bg-cyan-500 dark:bg-cyan-400 w-full h-2" style={{width: Math.floor(curTime) / Math.floor(duration) * 100 + "%"}} role="progressbar" aria-label="music progress"
                                 aria-valuenow={Math.floor(curTime)} aria-valuemin="0" aria-valuemax={Math.floor(duration)}></div>
                        </div>
                    ...
                    <div className="flex justify-between text-sm leading-6 font-medium tabular-nums">
                        <div className="text-cyan-500 dark:text-slate-100">{Math.floor(curTime)}</div>
                        <div className="text-slate-500 dark:text-slate-400">{Math.floor(Math.floor(duration) / 60) + ":" + (Math.floor(duration) - Math.floor(Math.floor(duration) / 60) * 60)}</div>
                    </div>
                </div>
            </div>
            <div className="bg-slate-50 text-slate-500 dark:bg-slate-600 dark:text-slate-200 rounded-b-xl flex items-center">
                <div className="flex-auto flex items-center justify-evenly">
                    <button type="button" className="hidden sm:block lg:hidden xl:block" aria-label="Previous" onClick={() => curTime > 15 ? setClickedTime(Math.floor(curTime) - 15) : setClickedTime(0)}>
                    ...
                    <button type="button" aria-label="Rewind 10 seconds" onClick={() => curTime > 15 ? setClickedTime(Math.floor(curTime) - 15) : setClickedTime(0)}>
                        <svg width="24" height="24" fill="none">
                            <path d="M6.492 16.95c2.861 2.733 7.5 2.733 10.362 0 2.861-2.734 2.861-7.166 0-9.9-2.862-2.733-7.501-2.733-10.362 0A7.096 7.096 0 0 0 5.5 8.226" stroke="currentColor" strokeWidth="2" strokeLinecap="round"
                ...
                <div className="flex-auto flex items-center justify-evenly">
                    <button type="button" aria-label="Skip 10 seconds" onClick={() => curTime + 15 < duration ? setClickedTime(Math.floor(curTime) + 15) : setClickedTime(duration)}>
                        <svg width="24" height="24" fill="none">
                            <path d="M17.509 16.95c-2.862 2.733-7.501 2.733-10.363 0-2.861-2.734-2.861-7.166 0-9.9 2.862-2.733 7.501-2.733 10.363 0 .38.365.711.759.991 1.176" stroke="currentColor" strokeWidth="2" strokeLinecap="round"
                                  strokeLinejoin="round"/>
